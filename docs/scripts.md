...
| `fm-role-profile-lib.sh`  | Resolve a role's harness/model/effort from `config/role-profiles.json` with override precedence; sourced helper |
| `fm-project-director.sh` | Drive the Project Director state machine: spec intake, DAG plan, dispatch, gap cycles, and acceptance |
